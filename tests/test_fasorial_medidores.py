...
def test_nestle_entra_a_la_lista_aunque_no_tenga_frontera(sv, monkeypatch):
    ...
    Ninguna frontera lo nombra —es un autoconsumo— y aun así tiene que salir en
    la respuesta, junto a los que sí resuelven por su código de frontera.
    """
    _montar(
        sv, monkeypatch,
    ...
    proyectos = sv.proyectos_con_medidor()["projects"]

    assert {"proyecto_id": 59, "nombre": "Nestlé DPA"} in proyectos
    assert {"proyecto_id": 7, "nombre": "Minigranja Solar Baraya"} in proyectos

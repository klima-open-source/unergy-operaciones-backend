...
def proyectos_con_medidor() -> dict:
    ...
    # Se le pregunta al resolvedor por CADA proyecto en vez de armar antes una
    # lista de candidatos. Así entra solo todo lo que él sepa resolver —incluido
    # `_PROYECTO_NODE_OVERRIDE`, que es como llegan los medidores sin border en
    # Gaia— sin que este módulo tenga que enumerar los overrides ni que haya que
    # agregarle nada a `app/`, que está congelado y se lee, no se extiende.
    # No cuesta llamadas externas: el mapa dinámico de Quoia se construye una
    # sola vez (cacheado 1 h) y lo demás es búsqueda en diccionario.
    gaia = _get_gaia()
    filas = [
